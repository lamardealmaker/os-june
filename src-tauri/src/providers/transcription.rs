...
pub const DEFAULT_TRANSCRIPTION_PROVIDER: &str = crate::providers::OPENAI_PROVIDER;
...
pub async fn transcribe_saved_audio(
    ...
    match request.provider.as_str() {
        crate::providers::OPENAI_PROVIDER => transcribe_with_openai(&request).await,
        crate::providers::VENICE_PROVIDER => transcribe_with_venice(&request).await,
        _ => Err(AppError::new(
            "provider_not_configured",
            "Unsupported transcription provider. Set OPENAI_API_KEY for OpenAI transcription.",
        )),
    }
}
...
async fn transcribe_with_openai(
    ...
    let response = reqwest::Client::new()
        .post(format!(
            ...
            crate::providers::openai_api_base_url()
        ))
        .bearer_auth(api_key)
        .multipart(form)
        .send()
